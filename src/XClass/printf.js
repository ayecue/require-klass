define(['require','exports','module','./forEach','./toArray','./extend'],function(require, exports, module, forEach, toArray,extend) {
    var /**
		 *	Variables
		 */
		rxFirst = '%(?::([^:]+):)?',
		...
		printf = function(string,object,value){
			if (typeof value == 'string') {
				return string.replace(new RegExp(rxFirst + object + rxLast, rxGlobal), function(toReplace,formatCmds){
					return formatCmds ? _forEach(formatCmds.split(','),function(_,cmd){
						this.result = format(this.result,cmd);
					},value) : value;
				}).replace(rxReplace,'');
			} else if (typeof object == 'object') {
				return _forEach(object,function(key,value){

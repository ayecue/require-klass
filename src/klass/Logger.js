		...
		$print: function(context,args,error,color){
			...
			if (context) {
				if (context.deepLoggingLevel || methodName == CONSTANTS.LOGGER.ANONYMOUS_NAME) {
					...
					console.groupCollapsed.apply(console,['%c[require-klass-logger] ' + contextName + '.' + methodName + messages.join(' '),color].concat(args));
					console.log('%c[require-klass-logger] ' + deepTrace,color);
					console.groupEnd();
				} else {
					console.log.apply(console,['%c[require-klass-logger] ' + contextName + '.' + methodName + messages.join(' '),color].concat(args));
				}
			} else {
				console.log.apply(console,['%c[require-klass-logger]' + messages.join(' '),color].concat(args));
			}
		},
